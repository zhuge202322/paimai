...
export default function ParallaxShowcase({ products = [], heroPost, hotSales = [], visionaries = [] }: ParallaxShowcaseProps) {
  ...
  const hotSalesItems = useMemo(() => {
    ...
    return hotSales.map((item: any) => {
        ...
        return {
            id: p.id,
            name: p.title,
            image: fixUrl(p.featuredImage?.node?.sourceUrl) || "/images/02.png",
            slug: p.slug
        };
    });
  }, [hotSales]);
  ...
  const heroData = useMemo(() => {
    ...
    // Fix relative URLs and port
    articleImage = fixUrl(articleImage);

    // Featured Image for "Right Picture"
    let featuredImage = fixUrl(heroPost.featuredImage?.node?.sourceUrl) || "/images/03.png";
  ...
  const scrollProducts = useMemo(() => {
    ...
    return products.map((item: any) => {
        ...
        return {
            ...
            image: fixUrl(p.featuredImage?.node?.sourceUrl) || "/images/bo/1.png",
